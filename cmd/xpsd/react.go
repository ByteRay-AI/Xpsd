...
type SessionUsage struct {
	Tokens   float64 // context-window tokens at the end of the session
	Requests int     // number of LLM turns (AssistantTurnEnd events)
	// ToolCalls is how many tool calls completed. It measures the work done,
	// where Requests measures what is billed.
	ToolCalls int
	// NanoAIU is the billed cost Copilot reported for this session, in
	// nano-AI-units, summed over its API calls. HasCost is false when the
	// runtime reported none, in which case the cost is unknown rather than zero.
	NanoAIU float64
	HasCost bool
	// PromptTokens approximates the input actually billed. An agent turn resends
	// the whole conversation, so this sums the context size observed at each
	// turn rather than reporting the final window size, which counts the context
	// once no matter how many turns re-sent it.
	PromptTokens float64
	// Model is the model the session actually ran on, as reported by the
	// runtime. Empty when no event named it.
	Model string
}
...
type ReactEnforcer struct {
	...
	// promptTokens accumulates the context size at each turn, and model records
	// what the runtime says it ran on.
	promptTokens float64
	toolCalls    int
	nanoAIU      float64
	hasCost      bool
...
func (r *ReactEnforcer) EventHandler() copilot.SessionEventHandler {
	return func(event copilot.SessionEvent) {
		...
		case *copilot.AssistantTurnEndData:
			...
			r.sawThought = false
			r.requests++
			// This turn sent the conversation as it stood, so bill it once.
			r.promptTokens += r.currentTokens
			if d.Model != nil && r.model == "" {
				r.model = *d.Model
			}

		case *copilot.AssistantUsageData:
			// Authoritative per-request billing from Copilot. Absent on other
			// providers and on endpoints that do not report it.
			if d.CopilotUsage != nil {
				r.nanoAIU += d.CopilotUsage.TotalNanoAiu
				r.hasCost = true
			}
