...
type SessionOpts struct {
	...
	// ToolAllow restricts the session to these MCP tool names. Empty means every
...
func RunSession(ctx context.Context, client *copilot.Client, opts SessionOpts, systemPrompt, userMessage string) (string, error) {
	...
	if len(opts.ToolAllow) > 0 {
		allow = make(map[string]bool, len(opts.ToolAllow))
		for _, n := range opts.ToolAllow {
			allow[n] = true
		}
	}
