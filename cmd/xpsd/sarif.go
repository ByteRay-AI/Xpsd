...
// ruleOutcome summarizes every result sharing a rule id. GitHub reads
// security-severity from the RULE and never from the result, so a rule backing
// several findings has to take the most serious outcome among them.
type ruleOutcome int

const (
	outcomeNotReachable ruleOutcome = iota // every result ruled it out
	outcomeNeedsReview                     // uncertain, unparseable, or errored
	outcomeReachable                       // at least one result is reachable
)

// notReachableSeverity puts a ruled-out finding in GitHub's "low" band
// (0.1 to 3.9) so it stops competing for attention with findings that are
// actually reachable. The real CVSS score stays in the rule description.
const notReachableSeverity = "1.0"

// resultOutcome classifies one analyzed finding.
func resultOutcome(r FindingResult) ruleOutcome {
	if r.Err != nil || r.Verdict == nil {
		return outcomeNeedsReview
	}
	switch r.Verdict.Reachable {
	case "yes":
...
// ruleDefaultLevel is the level applied to results that carry none.
func ruleDefaultLevel(outcome ruleOutcome) string {
	if outcome == outcomeNotReachable {
		return "note"
	}
	return "warning"
}
...
// uses to bucket alerts (critical > 9.0, high >= 7.0, medium >= 4.0, low >= 0.1).
// Falls back from a real CVSS score to a band-representative value.
//
// A rule whose every finding was ruled out drops to the low band: the CVSS score
// describes the vulnerability, not the risk it poses to this codebase, and a
// ruled-out critical shown as critical is what buries the reachable ones.
// Anything needing review keeps its real score.
func securitySeverity(f Finding, outcome ruleOutcome) string {
	if outcome == outcomeNotReachable {
		return notReachableSeverity
	}
	if f.CVSSScore > 0 {
...
func BuildScanSARIF(results []FindingResult, srcAbs, uriPrefix string) ([]byte, error) {
	...
	// GitHub takes security-severity from the rule, so a rule shared by several
	// findings has to be built from the most serious outcome among them. One
	// reachable finding keeps the whole rule at its real severity.
	outcomes := map[string]ruleOutcome{}
	for _, r := range results {
		if o := resultOutcome(r); o > outcomes[r.Finding.ID] {
			outcomes[r.Finding.ID] = o
		}
	}
	...
	for _, r := range results {
		...
		if !ok {
			idx = len(rules)
			ruleIndex[f.ID] = idx
			rules = append(rules, buildRule(f, outcomes[f.ID]))
		}
...
func buildRule(f Finding, outcome ruleOutcome) sarifRule {
	title := f.Title
	if title == "" {
	...
	if outcome == outcomeNotReachable {
		help.WriteString(" Alert severity is lowered because reachability analysis" +
			" found no path from external input to the vulnerable code in this" +
			" repository. The vulnerability's own rating is unchanged; see the" +
			" alert body for the reasoning and the evidence behind it.")
	}
	...
	return sarifRule{
		...
		HelpURI:              helpURI,
		DefaultConfiguration: &sarifRuleCfg{Level: ruleDefaultLevel(outcome)},
		Properties: &sarifRuleProps{
			Tags:             []string{"security", "reachability", "xpsd"},
			SecuritySeverity: securitySeverity(f, outcome),
		},
	}
}
