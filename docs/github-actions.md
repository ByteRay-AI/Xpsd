...
| Verdict | SARIF level | Alert |
|---------|-------------|-------|
| reachable (medium/high confidence) | `error` | severity from CVSS (`security-severity`) |
| reachable (low confidence), uncertain, analysis failed | `warning` | needs manual review |
| not reachable | `note` | informational |
...
| Pull of `ghcr.io/byteray-ai/xpsd:1` fails | No `v1.x.y` release published, or the GHCR package is private. |
